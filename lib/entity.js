...
Entity.prototype.get = function (id, connection, userParams) {
    ...
    return schemaProcessing.getSchema('userparams')
        .then(function (schema) {
            ...
            var queryString = T1Connection.buildQueryString(entitymap.get_endpoint(that.entity_type) + "/" + id, userParams);
            return connection.get(queryString);
        }).then(function (body) {
            var content = JSON.parse(body);
            return that.process_entity(content.data, content.meta);
        })
};


// called on successful save of entity
Entity.prototype.process_entity = function (data, meta) {
    for (var property in data) {
        if (data.hasOwnProperty(property)) {
            if (data[property].constructor === Array &&
                ...
                related.process_entity(data[property], {});
    ...
    return schemaProcessing.getSchema(that.entity_type)
        .then(function (schema) {
            ...
            if (verification.length != 0) {
                ...
                var endpoint = entitymap.get_endpoint(that.entity_type);

        ...
        if (!that.cookieAuthed && this.oauth2 === undefined) {
            ...
            if (that.oauth2Token) {
                if (that.oauth2Token.expired()) {
                    that.oauth2.authorizationCode.getToken(that.tokenConfig)
                        .then((result) => {
                ...
                BPromise.reject("No oauth2 token! Ensure the " +
                    "oauth2 flow is completed by calling fetchToken()");
            }
        }
    });
};
...
T1Connection.prototype.fetchAuthUrl = function () {
    var that = this;
    if (!that.oauth2) {
        that.initializeOauth();
    }
...
T1Connection.prototype.fetchToken = function (code) {
    var that = this;
    that.tokenConfig = {
        code: code,
        redirect_uri: that.t1config.redirect_uri
...
T1Connection.buildQueryString = function (baseUrl, userParams) {
    ...
    if (this.cookieAuthed) {
        userParams.api_key = this.t1config.api_key;
    }
    else if (this.oauth2) {
        userParams.api_key = this.t1config.client_id;
    }

...
T1Connection.prototype.initializeOauth = function () {
    var that = this;
    that.oauth2 = OauthService({
      clientID: that.t1config.api_key,
      clientSecret: that.t1config.secret,
      site: that.t1config.oauthUrl,
      tokenPath: '/token',
...
T1Connection.prototype.fetchAuthUrl = function (redirect_uri, scope, state) {
    var that = this;
    return that.oauth2.authCode.authorizeURL({
...
T1Connection.prototype.fetchToken = function (code, redirect_uri) {
    var that = this;
    return that.oauth2.authCode.getToken({
        code: code,
        redirect_uri: redirect_uri,
        clientID: that.t1config.api_key,
        clientSecret: that.t1config.secret,
  ...
  function saveToken(error, result) {
    ...
    var token = that.oauth2.accessToken.create(result);
    return token;
  }
};

...
T1Connection.prototype.ensureAuthenticated = function() {
    ...
    return BPromise.try(function() {
        if (!that.cookieAuthed && !that.oAuth2Token) {
            ...
            BPromise.reject("No Cookie or OAuth2 token. Ensure the " +
    ...
    that.oauth2.ownerPassword.getToken(tokenConfig)
        .then(
            (result) => {
                ...
                that.requestHeaders.Authorization = 'Bearer ' + that.oAuth2Token.token.id_token;
